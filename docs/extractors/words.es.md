...
!!! note "Nota"
    El tokenizador por defecto es el tokenizador por reglas de la clase de idioma español de [spaCy](https://github.com/explosion/spaCy) (`ests.utils.tokenize`); no necesita un modelo entrenado. Los signos de puntuación, incluidos `¿` y `¡`, los números como `1.500,50`, `3.º`, `1990-1995` y las abreviaturas como `Sr.`, `EE. UU.` son tokens únicos; las palabras con pronombres enclíticos (`dámelo`, `decírselo`) no se separan. Las rayas de un diálogo pegadas a las palabras, como las escriben los corpus de texto plano, se separan: `--No`, `-dijo`, `sí--dijo`, `reírse—me decía`, `dijo:—¡Mis` dan las palabras `No`, `dijo`, `sí`, `reírse`, `me`, `Mis`, mientras que un guion entre letras (`franco-alemán`) o ante una cifra (`-5`) se queda en su token; un sufijo citado con su guion (`-mente`) también lo pierde. `ests.utils.add_dash_rules(nlp)` da las mismas reglas a un pipeline de spaCy propio; el modelo de `get_nlp` ya las tiene.
