...
from ests.constants import COLLOCATION_MEASURES
from ests.corpus import Collocation, collocations
from ests.corpus.collocations import (
    MEASURES,
    calc_dice,
    calc_log_likelihood,
    calc_logdice,
...
def test_collocations():
    ...
    assert found[0] == Collocation("gato", "en", 3, 3, 3, 13.0)
...
def test_bigrams():
    assert collocations(words, window=1) == [
        Collocation("en", "ventana", 3, 2, 2, calc_logdice(3, 2, 2, 12)),
        Collocation("estaba", "en", 2, 3, 2, calc_logdice(2, 3, 2, 12)),
        Collocation("gato", "estaba", 3, 2, 2, calc_logdice(3, 2, 2, 12)),
    ]


def test_collocations_of_a_node():
    assert collocations(words, window=1, measure="t_score", node="gato") == [
        Collocation("gato", "estaba", 3, 2, 2, calc_t_score(3, 2, 2, 12))
    ]
    found = collocations(words, window=1, node="en", min_freq=1)
