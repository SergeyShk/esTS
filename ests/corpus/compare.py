...
from ..constants import MORPHOLOGY_STATS_DESC, OPENING_MARKS
...
from ..utils import check_sequence, iter_text_sents, iter_text_words
...
def split_windows(text: str, window: int | None = 1000) -> list[str]:
    ...
    Description:
        ...
        zeta: short texts are not lost and the tail is not dropped. The first
        window starts at the first non-space character of the text, a boundary
        between windows goes before the first word of the next window and the
        opening marks before it (OPENING_MARKS: quotes, brackets, dashes, the
        inverted ¿ and ¡), and the last window lasts to the end of the text;
        the windows are stripped of whitespace, so the punctuation before the
        first word, after the last word of a window and at the end of the
        text stays in the windows; with window=None the window is the whole
    ...
    Raises:
        ParameterError: If the size of a window is below one
...
def text_features(text: str, nlp: Language | None = None) -> dict[str, float]:
    ...
    Description:
        Features prefixed by their source: basic_ - the shares of unique,
        long, complex, simple, mono- and polysyllabic words, of letters,
        spaces and punctuation marks, the mean number of letters and syllables
        per word and of words per sentence (BasicStats); readability_ - every
        readability formula, the consensus grade and the reading time
        (ReadabilityStats); diversity_ - every measure of lexical diversity
        (DiversityStats); morph_ - the shares of the parts of speech among the
        words and the shares of the values within every morphological feature
        over the full list of values of MORPHOLOGY_STATS_DESC, for instance
        morph_pos_NOUN and morph_mood_Sub: a value that does not occur gives 0,
        a feature absent from the window altogether (no verbs - no tense) nan,
        and the markers of Spanish of MorphStats.get_markers, for instance
        morph_p_ser and morph_p_mente_adverbs; sents_ - the mean length of a
        ...
        once (ReadabilityStats gets the ready BasicStats). The morphology needs
        the parse of the model es_core_news_sm or of the pipeline in nlp,
        which takes most of the time: about 0.1 s for a window of 1000 words;
    ...
    Example:
        ...
        >>> features["basic_words_per_sent"], round(features["morph_pos_DET"], 3)
        (5.5, 0.273)
    """
    ...
    features.update((f"morph_{key}", float(value)) for key, value in morph.get_markers().items())
    features.update(
...
def corpus_features(
    ...
    Raises:
        ...
        SourceError: If the corpus has no words

    Example:
        >>> from ests.corpus import corpus_features
        >>> texts = ["El gato duerme. El perro come.", "Llueve."]
        >>> corpus_features(texts, window=3, features=lambda text: {"chars": len(text)})
                     chars
        text window
        0    0        15.0
             1        14.0
...
def compare_corpora(
    ...
    Description:
        The texts of both corpora are split into windows of the same size, so
        that the features do not depend on the length, the features are
        computed for every window (text_features or a function of one's own),
        and for every feature the two sets of values are compared: the means
        and the medians, the difference of the medians with its 95% percentile
        bootstrap interval, Cohen's d, Cliff's delta, the AUC of the feature as
        a classifier on its own (the share of the pairs of windows where the
        value in A is greater than in B, ties counted as half; Cliff's delta
        equals 2·AUC − 1), the Mann-Whitney U test with a two-sided p-value and
        Holm's correction for the number of features. Undefined and infinite
        values of a feature are dropped, and with fewer than two values on a
        side the statistics of the feature are nan. The rows are sorted by
        descending absolute Cliff's delta, the features without statistics
        go last
    ...
    Raises:
        SourceTypeError: If a string is passed instead of a list of texts
        SourceError: If one of the corpora has no words
...
def compare_features(
    ...
    Description:
        ...
        pairwise, for instance. A column missing from one of the tables is
        compared with an empty set of values and gives nan
...
def _finite(table: pd.DataFrame, name: str) -> np.ndarray:
    """Finite values of a column of a table; an empty array for a missing column"""
...
def bootstrap_median_diff(
    ...
    Returns:
        tuple[float, float]: Lower and upper bounds, nan for an empty set
